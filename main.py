...
class AppBottle(bottle.Bottle):
	def __init__(self):
		super().__init__()

		@self.route("/")
		def index():
			return bottle.template("index", name=NAME, lang=os_lang, LANG=LANG)

		@self.route("/css/style.css")
		def style():
			bottle.response.content_type = "text/css; charset=UTF8"
			return bottle.template("style")

		@self.route("/js/index.js")
		def style():
			bottle.response.content_type = "text/javascript; charset=UTF8"
			return bottle.template("jsindex", websocketport=websocketport, LANG=LANG)


class AppBottleWebsocket(bottle.Bottle, App):
	def __init__(self):
		super().__init__()

		@self.route("/get_servers_state", apply=[websocket])
		def get_servers_state(ws):
			log.debug("Get servers state channel open")
			while True:
				msg = ws.receive()
				if msg is not None:
					log.debug("Get servers state request")
					ws.send(json.dumps([i for i in self.get_servers_state()]))
				else:
					break

		@self.route("/servers_control", apply=[websocket])
		def servers_control(ws):
			log.debug("Get servers control channel open")
			while True:
				msg = ws.receive()
				if msg is not None:
					data = json.loads(msg)
					if data[0] == 0:
						log.debug(f"Get servers control start request: {data[1]}")
						self.start_server([cfg["servers"][i] for i in data[1]])
					elif data[0] == 1:
						log.debug(f"Get servers control stop request: {data[1]}")
						self.stop_server([cfg["servers"][i] for i in data[1]], data[2])
				else:
					break
...
def parse_args():
	...
	parser.add_argument('--usetk', action="store_true",
						help='Use Tkinter gui instead of web gui')
...
if __name__ == '__main__':
	if args.usetk:
		...
		websocketport = args.port + 1
		threading.Thread(target=lambda: bottle.run(AppBottleWebsocket(), host=args.host, port=websocketport,
												   server=GeventWebSocketServer)).start()
		appinstance = AppBottle()
		if not args.nobrowseropen:
			threading.Thread(target=browseropen).start()

		bottle.run(appinstance, host=args.host, port=args.port)
