...
class PluginsManagement(tk.Tk):
    def __init__(self, servers):
        ...
        self.install_from_file_btn = tk.Button(
            self.installed_topbar,
            text="Install plugin from file",
        ...
        self.search_btn = tk.Button(
            self.all_topbar, text="Search", command=self.get_page
        )
        self.search_btn.grid(column=1, row=0)
        self.page_spinbox = tk.Spinbox(
            self.all_tab, from_=1, to=255, command=self.get_page
        )
        self.page_spinbox.grid(
            column=int(self.page_columns / 2),
            row=int(self.page_size / self.page_columns) + 1,
        )
        self.get_page()
        self.mainloop()
                ...
                try:
                    ...
                    if id in self.installed_plugins_ids:
                        ...
                        try:
                            plugin = json.loads(
                                requests.get(
                                    "https://api.spiget.org/v2/resources/" + str(id)
                                ).text
                            )
                        except Exception as err:
                            log.error(
                                'Error "{0}" while parsing installed plugin {1}'.format(
                                    str(err), name
                                )
                            )
                            continue
                        ...
                        log.debug(
                            "Found installed plugin {0} with id {1}".format(name, id)
                        )
                        if int(version) < plugin["versions"][0]["id"]:
                            ...
                            frame = tk.Frame(
                                self.installed_tab, relief="raised", borderwidth=2
                            )
                        if len(name) > 20:
                        ...
                        tk.Label(
                            frame,
                            text=plugin["tag"],
                            font=("Liberation Sans Narrow", 10, "italic"),
                        ).pack()
                        tk.Label(frame, text=plugin["updateDateFormatted"]).pack()
                        tk.Label(
                            frame, text=plugin["releaseDateFormatted"], fg="gray"
                        ).pack()
                        frame.bind(
                            "<Button-1>",
                            lambda e, p=plugin, s=self.servers: Plugin(
                                p, s, self.install_plugin_from_file
                            ),
                        )
    ...
    def get_page(self):
        ...
        if search == "":
            self.page_data = json.loads(
                requests.get(
                    "https://api.spiget.org/v2/resources/free?size="
                    + str(self.page_size)
                    + "&page="
                    + self.page_spinbox.get()
                    + "&sort=-updateDate"
                ).text
            )
        else:
        ...
        for plugin in self.page_data:
            ...
            tk.Label(
                frame, text=plugin["tag"], font=("Liberation Sans Narrow", 10, "italic")
            ).pack()
            tk.Label(frame, text=plugin["updateDateFormatted"]).pack()
    ...
    def plugin_parse(self, plugin):
        ...
        plugin["updateDateFormatted"] = "{0}-{1}-{2}".format(
            str(struct_time.tm_mday), str(struct_time.tm_mon), str(struct_time.tm_year)
        )
        struct_time = time.localtime(plugin["releaseDate"])
        plugin["releaseDateFormatted"] = "{0}-{1}-{2}".format(
            str(struct_time.tm_mday), str(struct_time.tm_mon), str(struct_time.tm_year)
        )
    ...
    def install_plugin_from_file(self, name=None, id=None, version=None):
        files = filedialog.askopenfilenames(
            parent=self, filetypes=(("Plugins", "*.jar"), ("All files", "*.*"))
        )
        for s in self.servers:
            ...
            for p in files:
                ...
                for i in plugins:
                    if len(i.rsplit("-", maxsplit=2)) == 3:
                        ratio = similar(
                            os.path.basename(p),
                            i.rsplit("-", maxsplit=2)[0] + i[i.rindex(".") :],
                        )
                    else:
...
class Plugin(tk.Tk):
    def __init__(self, plugin, servers, install_plugin_from_file_func):
        ...
        self.plugin = json.loads(
            requests.get(
                "https://api.spiget.org/v2/resources/" + str(plugin["id"])
            ).text
        )
        ...
        tk.Label(self, text=self.plugin["name"], font=("Free Avant Garde", 24)).grid(
            column=0, row=0
        )
        ...
        tk.Label(
            self.dates_topbar, text="Update date: " + plugin["updateDateFormatted"]
        ).grid(column=0, row=0)
        tk.Label(
            self.dates_topbar, text="Release date: " + plugin["releaseDateFormatted"]
        ).grid(column=1, row=0)
        if not self.plugin["external"]:
            tk.Label(
                self.dates_topbar,
                text="Size: "
                + str(plugin["file"]["size"])
                + plugin["file"]["sizeUnit"],
            ).grid(column=2, row=0)
        ...
        if all_servers_supported:
            tk.Label(
                self.sec_topbar,
                text="Supported versions: " + " ".join(plugin["testedVersions"]),
            ).grid(column=0, row=0)
        else:
            tk.Label(
                self.sec_topbar,
        ...
        tk.Button(self.btns_topbar, text="Translate", command=self.desc_translate).grid(
            column=0, row=0
        )
        tk.Button(self.btns_topbar, text="Original", command=self.open_original).grid(
            column=1, row=0
        )
        self.desc = scrolledtext.ScrolledText(self, width=100)
        self.desc.insert(
            "insert", self.parse_desc(base64.b64decode(self.plugin["description"]))
        )
        self.desc["state"] = "disabled"
        self.desc.grid(column=0, row=5)
        self.name_pattern = (
            plugin["nameAscii"].replace("/", "|") + "-" + str(self.plugin["id"]) + "-"
        )
        self.load()
        self.bind(
            "<Return>", lambda e, p=self.plugin: log.debug("Plugin data: " + str(p))
        )
        self.mainloop()
    ...
    def parse_desc(self, desc):
        ...
        def parse_urls(text):
            try:
                ...
                a_end_index = (
                    text[a_href_end_index:].index("</a>") + a_href_end_index + 4
                )
    ...
    def install_plugin(self):
        if self.plugin["external"]:
            ...
            self.install_plugin_from_file(
                self.plugin["nameAscii"],
                self.plugin["id"],
                self.plugin["versions"][0]["id"],
            )
        else:
            data = requests.get(
                "https://api.spiget.org/v2/resources/{0}/download".format(
                    str(self.plugin["id"])
                )
            ).content
            for s in self.servers:
                with open(
                    os.path.join(
                        ...
                        "plugins",
                        self.name_pattern
                        + str(self.plugin["versions"][0]["id"])
                        + self.plugin["file"]["type"],
                    ),
                    "wb",
                ) as f:
                    f.write(data)
    ...
    def load(self):
        ...
        for s in self.servers:
            installed_versions += [
                int(i[len(self.name_pattern) : 0 - len(self.plugin["file"]["type"])])
                for i in os.listdir(os.path.join(s["dir"], "plugins"))
                if i.startswith(self.name_pattern)
            ]
        if installed_versions:
            if installed_versions[0] < self.plugin["versions"][0]["id"]:
                self.update_btn = tk.Button(
                    self, text="Update", bg="green", command=self.update_plugin
                )
            ...
            self.install_btn = tk.Button(
                self, text="Install", command=self.install_plugin
            )
    ...
    def open_original(self):
        webbrowser.open_new_tab(
            "https://spigotmc.org/resources/" + str(self.plugin["id"])
        )
